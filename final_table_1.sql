...
CREATE OR REPLACE VIEW v_andrea_zemanova_payroll_complete AS
SELECT 
	pay.payroll_year,
	pay.payroll_quarter, 
	ind.name AS industry,
...
;
-- rok 2000, 1Q, value = 11941, industry = NULL
...
CREATE OR REPLACE VIEW v_andrea_zemanova_avg_payroll_industry AS
SELECT 
	payroll_year,
	industry,
	industry_branch_code, 
...
CREATE OR REPLACE VIEW v_andrea_zemanova_food_date AS 
SELECT 
	pri.date_from AS 'year from',
	pri.date_to AS 'year to',
	pri.value AS price_in_CZK,
...
CREATE OR REPLACE VIEW v_andrea_zemanova_avg_food_year_14 AS 
SELECT 
	YEAR(`year from`),
	food_code,
	food,
...
CREATE OR REPLACE VIEW v_andrea_zemanova_avg_food_year_14_2 AS
SELECT 
	period,
	food_code,
	food,
...
SELECT 
	b.confirmed,
	a.avg_food_year
FROM v_andrea_zemanova_avg_food_year_14 AS a
JOIN v_andrea_zemanova_avg_food_year_14_2 AS b
	ON b.confirmed = a.avg_food_year 
;
-- 323 radku

SELECT 
	round ((b.confirmed - a.avg_food_year),1) AS difference,
	count(round ((b.confirmed - a.avg_food_year),1)) AS `count`
FROM v_andrea_zemanova_avg_food_year_14 AS a
JOIN v_andrea_zemanova_avg_food_year_14_2 AS b
	ON b.confirmed = a.avg_food_year 
;
...
CREATE OR REPLACE VIEW v_andrea_zemanova_food_avg_year AS
SELECT 
	`YEAR(``year from``)`  AS period,
	food_code,
	food,
...
-- napojeni tabulek pres sloupecky jednotlivych roku:
SELECT *
...
CREATE OR REPLACE TABLE t_andrea_zemanova_project_SQL_primary_final_join AS 
SELECT *
FROM v_andrea_zemanova_avg_payroll_industry AS ind
LEFT JOIN v_andrea_zemanova_food_avg_year AS food
	ON food.period = ind.payroll_year 
...
CREATE OR REPLACE TABLE t_andrea_zemanova_project_SQL_primary_final AS
SELECT * 
FROM t_andrea_zemanova_project_SQL_primary_final_join tazpspfj
WHERE period IS NOT NULL OR food_code IS NOT NULL OR food IS NOT NULL 
	OR price_value IS NOT NULL OR price_unit IS NOT NULL OR avg_food_year IS NOT NULL
